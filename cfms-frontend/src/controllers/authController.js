...
const login = catchAsync(async (req, res) => {
  ...
  if (looksLikeEmail) {
    ...
    // Phone numbers live on the Resident record, not on User directly —
    // normalize to digits only so formatting (spaces, dashes, +country
    // code) doesn't cause a false miss, then hop from resident -> user.
    const digits = identifier.replace(/[^\d]/g, '');
    const residents = await prisma.resident.findMany({ where: { phone: { not: null } }, include: { user: true } });
    const match = residents.find((r) => r.phone && r.phone.replace(/[^\d]/g, '').endsWith(digits) && digits.length >= 7);
